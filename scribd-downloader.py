...
def convert_scribd_link(url):
    ...
    Returns:
        ...
        can be extracted.
    """
    match = re.search(r"https?://(?:[a-zA-Z0-9_-]+\.)?scribd\.com/(?:document|doc)/(\d+)", url)
    if not match:
...
def save_pdf_pages_individually(
    ...
    # Ambil geometri master dari halaman 1 (atau halaman valid pertama)
    master_geom = driver.execute_script(
        """
        const pages = Array.from(document.querySelectorAll('.outer_page'));
        for (const p of pages) {
            const r = p.getBoundingClientRect();
            if (r.width >= 400 && r.height >= 500) {
                return { width: Math.ceil(r.width), height: Math.ceil(r.height) };
            }
        }
        return { width: 900, height: 1280 };
        """
    )
    master_w = master_geom.get("width", 900)
    master_h = master_geom.get("height", 1280)
    master_w_in = master_w / 96.0
    master_h_in = master_h / 96.0

    print(f"Master page size locked: {master_w}x{master_h}px ({master_w_in:.2f}\" x {master_h_in:.2f}\")")
    ...
    try:
        for index in range(page_count):
            ...
            driver.execute_script(
                ...
                const masterW = arguments[1];
                const masterH = arguments[2];

                const pages = Array.from(
                    document.querySelectorAll('.outer_page')
                );
                ...
                // Hapus HANYA overlay promosi & watermark luar (JANGAN hapus class blur karena gambar absimg ada di dalamnya)
                target.querySelectorAll('.view_on_scribd, .between_page_portal, .between_page_module, .promo_wrapper, .page_blur_promo_wrapper, button[class*="promo"]').forEach(el => el.remove());
                
                // Buka blur dan transparansi pada seluruh elemen gambar, teks, dan container
                ...
                style.textContent = `
                    @page {
                        size: ${masterW}px ${masterH}px;
                        margin: 0;
                    }

                    @media print {
                        html, body {
                            width: ${masterW}px !important;
                            height: ${masterH}px !important;
                            min-width: ${masterW}px !important;
                            min-height: ${masterH}px !important;
                            max-width: ${masterW}px !important;
                            max-height: ${masterH}px !important;
                            margin: 0 !important;
                            padding: 0 !important;
                            overflow: hidden !important;
                            -webkit-print-color-adjust: exact !important;
                        ...
                        /* Hilangkan balok hitam glitch pada text_layer spans */
                        .text_layer span,
                        .outer_page span,
                        .newpage span,
                        .orig_doc span {
                        ...
                        .outer_page[data-export-target="true"] img,
                        .outer_page[data-export-target="true"] .absimg,
                        .outer_page[data-export-target="true"] .orig_doc,
                        .outer_page[data-export-target="true"] .newpage {
            ...
            print(
                ...
                f"-> locked {master_w}x{master_h}px ({master_w_in:.2f}\"x{master_h_in:.2f}\")"
            )

            if PAGE_CB:
                try:
                    PAGE_CB(index + 1, page_count)
                except Exception:
                    pass
            ...
            result = driver.execute_cdp_cmd(
                "Page.printToPDF",
                {
                    ...
                    "scale": 1,
                    "paperWidth": master_w_in,
                    "paperHeight": master_h_in,
                    "marginTop": 0,
                    "marginBottom": 0,
                    "marginLeft": 0,
                    "marginRight": 0,
        ...
        writer = PdfWriter()
        try:
            # Re-encode JPEG adaptif efisien: Target ukuran ramah kuota ~10-15MB (standar Scribd Premium).
            # Mulai q60 (sangat tajam & jernih), lalu fallback q50, q40 jika file tebal.
            target = int(os.getenv("SCRIBD_MAX_PDF_BYTES", str(15 * 1048576)))
            quality_candidates = [
                int(x.strip()) for x in os.getenv("SCRIBD_JPEG_Q", "60,50,40").split(",") if x.strip()
            ]
            for q in quality_candidates:
                ...
                for page in writer.pages:
                    ...
                    except Exception as repack_error:
                        print(
                            f"  Recompress skipped on one page: "
                            f"{repack_error}"
                        )
                ...
                print(
                    f"  JPEG q{q}: {recompressed} images, "
                    f"{sz / 1048576:.1f}MB in {time.time() - t0:.0f}s"
                )
                if sz <= target:
                    break
                writer.close()  # siapkan writer baru utk kualitas berikutnya
                writer = PdfWriter()
        finally:
            writer.close()
